...
def get_shop_info(shop_url):
    ...
    QQ = re.search("\d+",re.sub("\s+","",selector.xpath('//a[@class="qq_img ml5 mt3"]/@title')[0]))[0] if len(selector.xpath('//a[@class="qq_img ml5 mt3"]/@title')) != 0 else ""
...
# 主函数>>>>>>>>>>>>>>>>>>>>>>>>>>
for lis_index in range(2,101):
    req = requests.get("http://shop.99114.com/list/pinyin/A_{}".format(lis_index),headers=header)
    html = req.text
