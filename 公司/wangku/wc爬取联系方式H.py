...
def get_shop_info(shop_url):
    ...
    QQ = re.search("\d+",re.sub("\s+","",selector.xpath('//a[@class="qq_img ml5 mt3"]/@title')[0]))[0] if selector.xpath('//a[@class="qq_img ml5 mt3"]/@title') != '联系 ' else ""
...
# 主函数>>>>>>>>>>>>>>>>>>>>>>>>>>
for lis_index in range(82,101):
    req = requests.get("http://shop.99114.com/list/pinyin/H_{}".format(lis_index),headers=header)
    html = req.text
    shops = re.findall(r'<a href="(http\:\/\/shop\.99114\.com/.+?)" target="_blank"><b>(.+?)</b></a>',html)
    #总长度
    print(len(shops))
    # for index,value in enumerate(shops[]):
    for index,value in enumerate(shops[:218]):
        #  获取->公司名/公司链接
        ...
        print(lis_index,index,shop_name, shop_url+">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>.")
