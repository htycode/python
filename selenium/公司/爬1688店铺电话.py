...
class 爬取1688_phone:
    ...
    def 爬取(self):
        ...
        for html in titles:
            ...
            # 获取_店铺名:
            name = html.get_attribute('title')
            print(name)

            phone = self.get_联系方式html_phone(contactinfo_link)
            print("手机电话:" + phone)
            sleep(2)
    ...
    # 获取电话号码
    def get_联系方式html_phone(self,contactinfo_link):
        # 获取联系方式网址源码
        req = requests.get(contactinfo_link, headers=self.headers)
        if re.findall(r"移动电话", req.text):
            phone = re.findall(r"移动电话.+?(1\d{10})", req.text, re.DOTALL)[0]
        else:
            phone = "**********************************************"
